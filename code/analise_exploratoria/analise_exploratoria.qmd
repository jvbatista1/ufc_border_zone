...
```{r}
rdplot(df2$mortes_violentas, df2$distancias_fronteira_interior, c = 0)
```
...
```{r}
# Instalar e carregar o pacote 'car' se ainda não estiver instalado
library(car)

# Criar um modelo linear com as variáveis moderadoras
model_vif <- lm(mortes_violentas ~ taxa_analfabetismo + taxa_desemprego + gini + pibpc + taxa_renda_pobre + taxa_trab_infantil + taxa_homens_jovens, data = df_norte)

# Calcular o VIF
vif_values <- vif(model_vif)

# Ver os valores de VIF
print(vif_values)
...
```{r}
df_norte <- df %>% filter(arcos == "Arco Norte")
# Modelo com múltiplos moderadores (taxa de analfabetismo, desemprego e PIB per capita)
...
for (var in seguranca_vars) {
  ...
  # Executar o modelo com bandwidth padrão
  rd_est <- rdrobust(y = df_norte[[var]],
...
```{r}
rdplot(y = df_norte$mortes_violentas, x = df_norte$distancias_fronteira_interior, c = 0, nbins = 50)
